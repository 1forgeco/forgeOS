...
const APPROVAL_OPTIONS = ['payment', 'purchase', 'submit form', 'send message', 'publish', 'delete', 'accept terms', 'create appointment', 'sign']
    ...
    <div className="creation-layout">
      ...
      <section className="creation-form-card">
        ...
        <div className="creation-fields">
          ...
          {step === 3 && <label><span>Actions that always need approval</span><div className="creation-approval-grid">{APPROVAL_OPTIONS.map((option) => { const selected = values(form.approvals).includes(option); return <button type="button" className={selected ? 'selected' : ''} aria-pressed={selected} onClick={() => setForm({ ...form, approvals: selected ? values(form.approvals).filter((item) => item !== option).join(', ') : [...values(form.approvals), option].join(', ') })} key={option}>{selected ? <Check size={12} /> : <Plus size={12} />}{option}</button> })}</div><div className="safety-callout"><ShieldCheck size={16} /><p>Passwords, CAPTCHA, two-factor authentication, payment details, and final purchases always require you, even if removed here.</p></div></label>}
