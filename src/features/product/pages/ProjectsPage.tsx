import { ArrowRight, Bot, Copy, Pause, Play, Plus, Search, Sparkles, Trash2 } from 'lucide-react'
  ...
  return (
    <main className="product-page projects-page">
      <div className="product-page-heading">
        <div><span className="product-eyebrow"><Sparkles size={12} /> Your workspace</span><h1>Agents</h1><p>Create, test, deploy, and monitor the browser agents your work depends on.</p></div>
        <Link className="primary-product-action" to="/templates"><Plus size={15} /> Create agent</Link>
      </div>
      <section className="workspace-summary">
        ...
        <article className="runtime-summary"><span><i /> Runtime</span><strong>Search ready</strong><small>Multi-step actions are in beta</small></article>
      </section>
        ...
        <section className="saved-agent-grid">
          {filtered.map((agent) => {
            ...
            return <article className="saved-agent-card" key={agent.id}>
              <Link className="saved-agent-open" to={`/app/${agent.id}`}><div className="saved-agent-top"><span style={{ background: template?.softAccent, color: template?.accent }}><Bot size={18} /></span><b>Open <ArrowRight size={12} /></b></div><span className={`agent-status ${agent.status}`}><i /> {agent.status}</span><h3>{agent.name}</h3><p>{agent.goal}</p><div className="agent-site">{agent.websiteUrl.replace(/^https?:\/\/(www\.)?/, '').split('/')[0]}</div></Link>
