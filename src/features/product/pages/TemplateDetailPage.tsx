...
export function TemplateDetailPage() {
  ...
  return <main className="template-detail-page">
    ...
    <section className="template-detail-grid"><article><span className="product-eyebrow"><Sparkles size={11} /> What it does</span><h2>Starts useful. Stays editable.</h2><p>This card creates a complete workflow with a website, outcome, run inputs, browser capabilities, approval rules, fallback, and result format. Every part remains editable on the visual canvas.</p><ul>{template.capabilities.map((item) => <li key={item}><Check size={12} /> {item}</li>)}</ul></article><article className="template-example-card"><small>Example goal</small><p>{template.defaultGoal}</p><div><span>Example sites</span>{template.exampleSites.map((site) => <b key={site}>{site}</b>)}</div></article></section>
